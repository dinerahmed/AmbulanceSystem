...
public class registration implements Initializable {
    ...
    public void reg_complete(ActionEvent event) throws IOException {
        ...
        if (phn_number.getText().isBlank() || repass.getText().isBlank() || pass.getText().isBlank() || email.getText().isBlank()
                || area.getValue() == null || sub_area.getValue() == null || v_no.getText().isBlank() || first_name.getText().isBlank() ||
                last_name.getText().isBlank()) {
            msg_error.setText("Fill all the boxes");
        } else if (!check) {
            msg_error.setText("Invalid Email");
        } else if (!check2) {
            msg_error.setText("Invalid phone number");
    ...
    public boolean create_user() throws IOException {
        ...
        String tv = v_no.getText();


        File user_folder = new File("D:/Java Soft/diner_ambulance/");
        ...
        File userFile = new File("D:/Java Soft/diner_ambulance/" + tlast);
        boolean is_user_file_created = userFile.mkdir();

        File infoFile = new File("D:/Java Soft/diner_ambulance/" + tlast + "/info.txt");
